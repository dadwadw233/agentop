# Claude Code Usage Tracking 内部实现分析

## 📊 概述

Claude Code 使用**本地文件系统**来追踪使用情况，完全在客户端进行统计，不依赖实时API调用。

## 🗂️ 数据存储架构

### 核心目录结构

```
~/.claude/
├── stats-cache.json          # 📊 聚合统计缓存（核心）
├── history.jsonl             # 📜 命令历史记录
├── projects/                 # 📁 项目级会话存储
│   └── {project-path}/
│       └── {session-id}.jsonl  # 会话详细日志
├── debug/                    # 🐛 调试日志
├── file-history/             # 📄 文件操作历史
├── shell-snapshots/          # 📸 Shell状态快照
└── settings.json             # ⚙️  用户设置
```

## 🎯 stats-cache.json - 核心统计文件

### 文件结构

```json
{
  "version": 1,
  "lastComputedDate": "2026-01-13",

  // 1️⃣ 每日活动统计
  "dailyActivity": [
    {
      "date": "2026-01-13",
      "messageCount": 1073,      // 消息数量
      "sessionCount": 67,        // 会话数量
      "toolCallCount": 213       // 工具调用次数
    }
  ],

  // 2️⃣ 每日模型Token使用
  "dailyModelTokens": [
    {
      "date": "2026-01-13",
      "tokensByModel": {
        "claude-haiku-4-5-20251001": 41735,
        "claude-sonnet-4-5-20250929": 99436
      }
    }
  ],

  // 3️⃣ 模型累计使用（全部时间）
  "modelUsage": {
    "claude-sonnet-4-5-20250929": {
      "inputTokens": 79198,                    // 输入token
      "outputTokens": 1074415,                 // 输出token
      "cacheReadInputTokens": 418184691,       // 缓存读取token
      "cacheCreationInputTokens": 32663922,    // 缓存创建token
      "webSearchRequests": 0,                  // Web搜索请求数
      "costUSD": 0,                            // 费用（美元）
      "contextWindow": 0,
      "maxOutputTokens": 0
    }
  },

  // 4️⃣ 总体统计
  "totalSessions": 257,
  "totalMessages": 9507,
  "longestSession": {
    "sessionId": "6f06f9b8-f54e-46ea-a409-01e383be9d9f",
    "duration": 700213775,       // 毫秒
    "messageCount": 4657,
    "timestamp": "2025-12-04T13:33:41.984Z"
  },
  "firstSessionDate": "2025-12-04T13:33:41.984Z",

  // 5️⃣ 使用时段分布（按小时）
  "hourCounts": {
    "0": 2,   // 0点: 2次会话
    "3": 68,  // 3点: 68次会话（你的高峰期！）
    "15": 52, // 15点: 52次会话
    ...
  }
}
```

### 数据特点

✅ **包含的数据**：
- 精确的输入/输出token统计
- 缓存token统计（prompt caching）
- 按模型分类的使用量
- 按日期聚合的历史数据
- 会话和消息计数
- 工具调用统计

❌ **不包含的数据**：
- 实时quota信息（5小时/月度限额）
- 剩余quota
- 实际花费金额（costUSD字段为0）
- 小时级别的详细时间序列

## 📝 会话级追踪 - JSONL日志

### 1. history.jsonl - 全局命令历史

```json
{
  "display": "ls",
  "pastedContents": {},
  "timestamp": 1764859780718,
  "project": "/Users/yuyuanhong/projects/playground",
  "sessionId": "fb519122-83d2-42e2-9655-8572479c70c1"
}
```

**用途**：记录所有执行的命令，用于历史回溯。

### 2. 项目会话日志 - {session-id}.jsonl

**路径**：`~/.claude/projects/{project-path}/{session-id}.jsonl`

**示例结构**：
```json
{
  "parentUuid": null,
  "isSidechain": false,
  "userType": "external",
  "cwd": "/Users/yuyuanhong/projects/agent-monitor",
  "sessionId": "e0bab6cd-5809-47b3-b7ca-7b817a4613f1",
  "version": "2.1.6",
  "gitBranch": "",
  "type": "system",
  "subtype": "local_command",
  "content": "...",              // 实际内容
  "level": "info",
  "timestamp": "2026-01-13T19:58:42.022Z",
  "uuid": "d1cd4a1f-c768-4832-9570-cf42d696c21b",
  "isMeta": false
}
```

**特点**：
- 每行一个JSON对象（JSONL格式）
- 记录完整的会话上下文
- 包含消息内容、工具调用、系统事件
- 用于会话恢复和历史查看

## 🔄 统计更新机制

### 更新流程

```
┌─────────────────┐
│  用户交互       │
│  (消息/工具调用) │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  会话日志写入    │
│  {session}.jsonl│ ← 实时写入
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  后台聚合任务    │
│  (定期触发)     │ ← 可能是定时任务或会话结束时
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  更新统计缓存    │
│  stats-cache.json│ ← 批量更新
└─────────────────┘
```

### 更新时机

根据观察，`stats-cache.json` 的 `lastComputedDate` 显示为 "2026-01-13"，而今天是 1月14日，说明：

1. **不是实时更新**：统计有延迟（通常是一天）
2. **批量处理**：可能在会话结束或每日定时任务时更新
3. **异步聚合**：不影响实时性能

## 💡 Token统计实现

### Claude Code如何统计Token

**来源**：Claude API响应

每次API调用返回的响应包含token统计：

```json
{
  "usage": {
    "input_tokens": 1234,
    "output_tokens": 5678,
    "cache_creation_input_tokens": 100,
    "cache_read_input_tokens": 2000
  }
}
```

**处理流程**：

```python
# 伪代码示例
async def track_api_call(model, request, response):
    # 1. 从API响应提取token
    usage = response.usage

    # 2. 累加到会话统计
    session_stats.add_tokens(
        model=model,
        input_tokens=usage.input_tokens,
        output_tokens=usage.output_tokens,
        cache_read=usage.cache_read_input_tokens,
        cache_creation=usage.cache_creation_input_tokens
    )

    # 3. 写入会话日志
    write_jsonl(session_log, {
        'type': 'api_call',
        'model': model,
        'usage': usage,
        'timestamp': now()
    })

    # 4. 异步更新聚合统计（批量处理）
    schedule_stats_update()
```

### Prompt Caching的特殊处理

Claude Code 特别追踪了**缓存token**：

- `cacheReadInputTokens`：从缓存读取（价格便宜）
- `cacheCreationInputTokens`：创建缓存（首次成本）

**你的实际使用**：
```json
"claude-sonnet-4-5-20250929": {
  "inputTokens": 79198,              // 直接输入
  "outputTokens": 1074415,           // 输出
  "cacheReadInputTokens": 418184691, // 😱 巨量缓存读取！
  "cacheCreationInputTokens": 32663922
}
```

**分析**：你的缓存读取量是直接输入的**5000倍**！这说明Claude Code大量使用了prompt caching来优化成本。

## 📈 统计数据的精度

### ✅ 高精度数据

- **Token计数**：100%准确（直接来自API）
- **会话/消息计数**：100%准确（本地记录）
- **模型使用**：100%准确（按模型分类）

### ⚠️  估算数据

- **每日token**：只有总量，无输入/输出分解
- **费用**：`costUSD`字段为0（可能隐私考虑）
- **5小时quota**：完全不追踪

### ❌ 缺失数据

- **实时quota使用**：API不返回
- **Quota剩余量**：API不返回
- **精确的小时级数据**：只有会话计数，无token

## 🔍 为什么API不返回Quota信息？

### 技术原因

1. **分布式架构**：Quota在后端独立服务管理
2. **安全考虑**：避免客户端操纵quota检查
3. **性能优化**：不在每次API响应中计算quota

### 设计哲学

```
Anthropic的设计原则：
┌─────────────────────────────┐
│  API层：专注返回内容        │
│  ✓ Token统计                │
│  ✓ 缓存命中率               │
│  ✓ 模型信息                 │
└─────────────────────────────┘
         │
         ▼
┌─────────────────────────────┐
│  后端层：Quota管理分离      │
│  • Rate limiting            │
│  • Quota enforcement        │
│  • 仅在超限时拒绝请求       │
└─────────────────────────────┘
```

用户需要通过**Web Dashboard**查看quota，而非API。

## 🛠️ Claude Code的实现细节（推测）

### 架构模式

```typescript
// 可能的实现架构

class UsageTracker {
  private sessionLog: JSONLWriter;
  private statsCache: StatsCache;

  async trackInteraction(interaction: Interaction) {
    // 1. 实时写入会话日志
    await this.sessionLog.append({
      timestamp: Date.now(),
      type: interaction.type,
      content: interaction.content,
      model: interaction.model
    });

    // 2. 如果是API调用，记录token
    if (interaction.type === 'api_response') {
      this.statsCache.incrementTokens(
        interaction.model,
        interaction.usage.input_tokens,
        interaction.usage.output_tokens
      );
    }
  }

  async computeDailyStats() {
    // 定期任务：聚合会话日志到stats-cache.json
    const sessions = await this.loadAllSessions(today);
    const aggregated = this.aggregate(sessions);
    await this.statsCache.save(aggregated);
  }
}
```

### 文件系统操作

- **JSONL追加写入**：高效的日志追加（O(1)）
- **批量读取聚合**：定期读取所有会话日志，聚合到缓存
- **原子性更新**：使用临时文件+重命名保证stats-cache.json完整性

## 📊 与Agent Monitor的对比

| 功能 | Claude Code | Agent Monitor |
|------|-------------|---------------|
| **数据来源** | 本地JSONL + API响应 | 读取stats-cache.json |
| **Token统计** | 直接从API获取（精确） | 从缓存读取 |
| **实时性** | 会话级实时 | 依赖缓存更新（延迟） |
| **Quota追踪** | ❌ 不追踪 | ✅ 用户配置估算 |
| **历史数据** | ✅ 完整JSONL日志 | ✅ 读取聚合统计 |
| **5小时窗口** | ❌ 无 | ✅ 估算实现 |

## 🎯 关键发现

### 1. Claude Code不提供Quota API

```python
# ❌ 这样的API不存在
quota_info = claude.get_quota()

# ✅ 只能在Dashboard查看
# https://console.anthropic.com
```

### 2. stats-cache.json是唯一的本地统计源

这就是为什么Agent Monitor必须：
- 读取这个文件获取使用数据
- 用户手动配置quota限制
- 估算5小时窗口（无实际数据）

### 3. 缓存Token占比巨大

你的统计显示：
```
缓存读取: 418M tokens
直接输入: 79K tokens
比例: 5279:1 ⚡
```

这意味着Claude Code通过prompt caching节省了**巨额成本**！

## 💰 成本优化策略

基于你的数据：

```python
# 你的实际使用（全部时间）
sonnet_usage = {
    'input': 79_198,
    'output': 1_074_415,
    'cache_read': 418_184_691,
    'cache_creation': 32_663_922
}

# 如果没有缓存，成本对比
without_cache = (79198 + 418184691) / 1_000_000 * 3  # input价格
with_cache = (79198 / 1_000_000 * 3 +                # input
              418184691 / 1_000_000 * 0.3)           # cache read (10折)

print(f'无缓存成本: ${without_cache:.2f}')
print(f'有缓存成本: ${with_cache:.2f}')
print(f'节省: ${without_cache - with_cache:.2f}')
```

**预估节省**：使用prompt caching可能节省了**90%+的输入token成本**！

## 🔮 未来改进方向

如果Anthropic未来提供Quota API：

```python
# 理想的API设计
response = client.messages.create(
    model="claude-sonnet-4",
    messages=[...]
)

# 期望返回的quota信息
print(response.quota_info)
# {
#   "five_hour_window": {
#     "limit": 500000,
#     "used": 45200,
#     "remaining": 454800,
#     "reset_at": "2026-01-14T18:30:00Z"
#   },
#   "monthly": {
#     "limit": 5000000,
#     "used": 461270,
#     "remaining": 4538730
#   }
# }
```

那么Agent Monitor就可以直接显示**精确的quota信息**，而不需要用户手动配置！

## 📚 总结

Claude Code的usage tracking是一个**客户端聚合系统**：

1. **实时记录**：每次交互写入JSONL日志
2. **API数据**：从Claude API响应获取精确token统计
3. **批量聚合**：定期将日志聚合到stats-cache.json
4. **缓存优化**：大量使用prompt caching降低成本
5. **无Quota API**：quota信息仅在Web Dashboard可见

Agent Monitor通过读取stats-cache.json获取使用数据，并实现用户配置的quota追踪功能，弥补了API的不足。

---

**关键文件位置**：
- 统计缓存：`~/.claude/stats-cache.json`
- 会话日志：`~/.claude/projects/{project}/{session}.jsonl`
- 命令历史：`~/.claude/history.jsonl`

**Agent Monitor的价值**：
- ✅ 实时可视化（TUI）
- ✅ Quota估算和告警
- ✅ 进程监控
- ✅ 成本追踪
